...
export function dressEnvironment(THREE, scene, loc) {
  ...
  for (const mesh of [trunks, crowns, upper]) { mesh.castShadow = true; mesh.receiveShadow = true; scene.add(mesh); }

  // A readable visitor path connects the spawn to each learning station.
  const pathMaterial = mat(loc.id === 'farm' ? 0xc6b590 : 0xd2c9b7);
  for (const [x, z] of loc.stationPos) {
    ...
    path.position.set(x / 2, .025, (z + 8) / 2); path.receiveShadow = true; scene.add(path);
  }
