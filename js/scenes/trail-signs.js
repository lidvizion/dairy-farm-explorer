...
export function makeTrailSigns(THREE,B,labels) {
  ...
  const canvas=document.createElement('canvas');canvas.width=1024;canvas.height=Math.ceil(labels.length/2)*352;
  ...
  labels.forEach((text,i)=>{
    ...
    ctx.font='600 90px Georgia';ctx.fillText(i<labels.length-1?String(i+1):'?',x+256,y+110);
  ...
  return (index,color)=>{
    ...
    for(let i=0;i<uv.count;i++)uv.setXY(i,(index%2+uv.getX(i))/2,(Math.ceil(labels.length/2)-1-Math.floor(index/2)+uv.getY(i))/Math.ceil(labels.length/2));
